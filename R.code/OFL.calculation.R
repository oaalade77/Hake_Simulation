...
x.surv.normal <- seq(from=surv.bounds[1],to=surv.bounds[2],length=surv.nbins)   
...
# lines(x.surv.normal, surv.cdf,type='p',pch=4,col='blue',cex=0.5)
# lines(x.surv.normal, cumsum(sc.surv.normal), type='p',pch=3,col='black',cex=0.5)


### Exploitation index: Two options
# 1) Create normal distribution and cdf from 3-year moving average
...
x.exploit.normal <- seq(from=exploit.bounds[1],to=exploit.bounds[2],length=exploit.nbins)   
...
# If using the AIM data
catch.mat <- as.matrix(aim.data$'RelF.kt.kg') %*% t(as.matrix(x.surv.normal))  

# Testing with excel survey estimates  
# pop.size.excel <- c(2.618674232,2.742172609,2.826851424,2.893118268,2.948500722,2.996659173,3.03966588,3.078815039,3.114972928,3.148750303,3.180595637,3.21084929,3.239776789,3.267590254,3.294462707,3.320537966,3.345937666,3.370766387,3.395115469,3.419065938,3.44269078,3.466056764,3.489225936,3.512256878,3.5352058,3.558127533,3.581076456,3.604107397,3.627276569,3.650642554,3.674267395,3.698217864,3.722566946,3.747395667,3.772795368,3.798870626,3.82574308,3.853556544,3.882484044,3.912737696,3.944583031,3.978360405,4.014518294,4.053667454,4.096674161,4.144832612,4.200215065,4.266481909,4.351160724,4.474659101,4.75996259)  
# catch.mat <- as.matrix(aim.data$'RelF.kt.kg') %*% t(as.matrix(pop.size.excel))  

# If not using the AIM data  
# catch.mat <-  as.matrix(x.exploit.normal) %*% t(as.matrix(x.surv.normal))  

 
### Use associated probabilities for survey and relative F to generate matrix of joint probabilities associated with each possible catch
####### LEFT OFF HERE ######################
